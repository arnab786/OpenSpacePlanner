...
export class SessionService {
  ...
  public async updateTopic(topic: Topic): Promise<Topic> {let request: any;
      if (topic.id != null) {
      request = this.http.put(`${environment.apiUrl}/api/sessions/${this.currentSession.id}/topics/${topic.id}`, topic);
      }
      else {
      request = this.http.post(`${environment.apiUrl}/api/sessions/${this.currentSession.id}/topics/`, topic);
      }
