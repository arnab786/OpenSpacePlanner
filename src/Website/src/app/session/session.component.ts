...
export class SessionComponent implements OnInit, OnDestroy {
  ...
  public get assignedTopics(): Topic[] {
    return this.session.topics
      .filter((t) => t.roomId != null || t.slotId != null)
      .sort((a, b) => b.owner?.localeCompare(a.owner ?? '') || b.name?.localeCompare(a.name))
      .reverse();
  ...
  public AssignTopics(event: Event) {
    event.stopPropagation();
    let roomAssigned: boolean = false;
    for (let i = 0; i <= this.unassignedTopics.length;) {
      roomAssigned = false;
      for (let j = 0; i <= this.rooms.length; j++) {
        if (this.unassignedTopics[i].slots == this.rooms[j].seats) {
          if (this.unassignedTopics[i].slots <= this.slots.length) {
            for (let m = 0; m <= this.slots.length; m++) {
              if (this.assignedTopics.length > 0) {
                for (let n = 0; n < this.assignedTopics.length; n++) {
                  if (this.slots[m].id != this.assignedTopics[n].slotId) {
                    var slotId = this.slots[m].id;
                    this.updateTopic(this.unassignedTopics[i], this.rooms[j].id, slotId != null ? slotId : null);
                    this.assignedTopics.push(this.unassignedTopics[i]);
                    roomAssigned = true;
                    break;
                  }
                }
